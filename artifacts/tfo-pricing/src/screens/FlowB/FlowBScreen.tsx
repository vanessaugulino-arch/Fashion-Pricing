...
export default function FlowBScreen() {
  ...
  // Perfil de marca (pre-filled from FlowD store, editable locally)
  const [perfilMarcaLocal, setPerfilMarcaLocal] = useState(perfilMarcaStore || '');
  const handleSetPerfil = (p: string) => {
    setPerfilMarcaLocal(p);
    setPerfilMarca(p);
  };
  ...
  const resultData = useMemo(() => {
    ...
    despesasPorCategoria, localDespesas, perfilMarcaLocal,
  ]);
