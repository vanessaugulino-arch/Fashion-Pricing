...
function getPerfilMarca(margemBruta: number): string {
  if (margemBruta > 65) return 'luxo';
  if (margemBruta >= 55) return 'contemporanea';
  if (margemBruta >= 45) return 'basico';
  return 'acessivel';
}

const PERFIL_LABELS: Record<string, string> = { luxo: 'Luxo / Premium', contemporanea: 'Contemporânea', basico: 'Básico', acessivel: 'Acessível' };
...
export default function FlowBResult({ resultData, onBack, setActiveFlow }: Props) {
  ...
  const perfil = getPerfilMarca(margemBruta);
