...
export default function FlowAResult({ nomeProduto, precoInicial, custoInicial, icmsNum, segmento, canal, onBack, onReset }: Props) {
  const { setActiveFlow, salvarProduto } = useToolStore();
  ...
  const initPrecoMinViavel = minRef / 100 < 1 ? custoInicial / (1 - minRef / 100) : null;

  const origDiagnosis = diagnosisFlowA(initMargem, segmento, canal);
  const origBadgeStatus = initMargem > maxRef ? 'excellent' : initMargem >= minRef ? 'ok' : initMargem < minRef - 10 ? 'critical' : 'warning';
  const origBadgeLabel = initMargem > maxRef ? 'Acima da média' : initMargem >= minRef ? 'Dentro da média' : 'Abaixo da média';
  const origMargemColor = initMargem >= minRef ? '#2D6A4F' : initMargem < minRef - 10 ? '#991B1B' : '#B45309';
  ...
  const barMax = Math.max(maxRef * 1.6, initMargem * 1.2, 80);
  const minPos = (minRef / barMax) * 100;
  const maxPos = (maxRef / barMax) * 100;
  ...
  const simBadgeStatus = simMargemPct > maxRef ? 'excellent' : simMargemPct >= minRef ? 'ok' : simMargemPct < minRef - 10 ? 'critical' : 'warning';
  const simBadgeLabel = simMargemPct > maxRef ? 'Acima da média' : simMargemPct >= minRef ? 'Dentro da média' : 'Abaixo da média';
  const simMargemColor = simMargemPct >= minRef ? '#2D6A4F' : simMargemPct < minRef - 10 ? '#991B1B' : '#B45309';
        ...
        <div className="mb-5">
          ...
          <div className="flex items-start justify-between text-[11px] font-sans text-gray-400">
            ...
            <span className="text-center px-2">
              Referência: {minRef}%–{maxRef}% ({segmentoLabel.toLowerCase()} · {canal})
            </span>
